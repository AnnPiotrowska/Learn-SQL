...
SELECT SUM(poster_qty) AS total_poster_amount,
	   SUM(standard_qty) AS total_standard_amount
FROM orders;
...
SELECT w.channel, COUNT(occurred_at) number_of_times
FROM web_events w
GROUP BY w.channel
...
Your final table should have three columns - the name of the sales rep, the channel, and the number of occurrences. /liczba wystapien/
